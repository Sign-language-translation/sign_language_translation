...
def create_model(pkl_file_name = None, model_filename = None, models_folder_path = "models"):
    ...
    if not pkl_file_name:
        f'label_encoder_3d_rnn_cnn_{len(labels) // len(set(labels))}_vpw.pkl'

    if not pkl_file_name.endswith('.pkl'):
        pkl_file_name = pkl_file_name + '.pkl'

    save_label_mapping(label_encoder, models_folder_path + "/" + pkl_file_name)
    ...
    model.save(models_folder_path + "/" + model_filename)
